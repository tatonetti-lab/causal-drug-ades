...
```{r}
onsides_latest_release = read_csv('./adverse_reactions_active_labels.csv.gz') %>%
...
#   inner_join(pryan_set, by = c('ingredients_names'='drug_name')) %>% 
#   inner_join(meddra_reference_map, by = c('condition_name'='condition_name',
#                                           'cohort_id' = 'cohort_id')) %>%
#   pivot_longer(cols = matches('meddra_term|concept_name')) %>% select(-condition_name, -cohort_id, -meddra_pt) %>%
#   inner_join(meddra_reference_map, by = c('value'='concept_name')) %>% 
#   distinct(set_id, pt_meddra_id, meddra_pt, ingredients_rxcuis, ingredients_names,
#            drug_concept_id, affect, meddra_term = value, condition_name, cohort_id) %>%
#   group_by(condition_name) %>%
#   summarise(n_distinct(ingredients_names))

temp <- onsides_latest_release %>%
  inner_join(meddra_reference_map, by = c('pt_meddra_term'='concept_name',
                                           'pt_meddra_id'='meddra_pt')) %>%
  distinct(cohort_id, condition_name,
           drug_name = ingredients_names, concept_name = pt_meddra_term,
           meddra_pt = pt_meddra_id)
```
```{r}
pryan_set %>% 
  inner_join(meddra_reference_map) %>% 
  full_join(temp) %>% filter(affect == 1) %>% 
  group_by(condition_name, drug_name) %>%
  summarise(n = n()) %>%
  group_by(condition_name, n) %>%
  summarise(num = n())
```
...
```{r}
adr_section = read_csv('/Users/undinagisladottir/Documents/Columbia/Tatonetti_Lab/20231113_onsides/adverse_reactions_active_labels.csv') %>%
...
adr_condition_name = adr_section %>%
  ...
  select(condition_name, cohort_id, pt_meddra_term, pt_meddra_id, ingredients_names, ingredients_rxcuis) %>%
  distinct() %>%
...
reference_set = read_csv('data/reference_set_labels.csv') %>%
    mutate(drug_name = str_to_lower(drug_name))
reference_set %>% distinct(condition_name)
```

```{r}
adr_condition_name %>%
  filter(!is.na(condition_name)) %>%  distinct(condition_name, ingredients_names) %>%
  group_by(condition_name) %>% summarise(n())

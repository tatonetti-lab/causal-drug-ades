...
pryan_set %>% filter(drug_name %in% remove_negative_controls) %>% arrange(drug_name)
...
remove_pairs <- tibble('drug_name' = c('lisinopril', 'diflunisal', 'fenoprofen',
                           ...
                           'condition_name' = c('acute kidney injury',
                                                ...
                                                'acute myocardial infarction',
                                                'acute liver injury'))
remove_negative_controls <- unlist(str_split(pattern = "\\s+", 'entecavir				
tinidazole				
clozapine				
griseofulvin				
darunavir				
...
pryan_set %>% group_by(condition_name, affect) %>% summarise(ref_drugs = n_distinct(drug_concept_id))
```
...
onsides_latest_release %>%
  ...
  inner_join(pryan_set %>% filter(affect == 1), by = c('condition_name', 'ingredients_names'='drug_name')) %>%
  group_by(condition_name) %>%
  summarise(num_drugs = n_distinct(drug_concept_id)) %>%
  inner_join(pryan_set %>% filter(affect == 1) %>% group_by(condition_name) %>% summarise(ref_drugs = n_distinct(drug_concept_id))) %>%
  mutate(prop = num_drugs/ref_drugs)
```

#### Get missing positive controls
...
onsides_latest_release %>%
  ...
  mutate(condition_name = str_to_lower(condition_name),
         onsides = 1) %>% 
  right_join(pryan_set %>% filter(affect == 1), by = c('condition_name', 'ingredients_names'='drug_name')) %>%
...
onsides_latest_release %>%
  ...
  select(ingredients_names, condition_name, pt_meddra_term, pt_meddra_id) %>%
  mutate(condition_name = str_to_lower(condition_name)) %>%
  inner_join(pryan_set %>% filter(affect == 0), by = c('condition_name', 'ingredients_names'='drug_name')) %>% head()
